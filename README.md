...
токен соответствующего контура. Токены читаются только из окружения. SDK вынесен
в optional extra, чтобы mock/Docker runtime не зависел от недоступного broker
artifact: для T-Invest окружения установите `pip install -e ".[tinvest]"` через
package index, в котором доступна закреплённая версия SDK.
...
- `GET /api/v1/admin/automation/runs`;
...
ARQ worker исполняет единый `AutomationCycleService`; отдельный scheduler только
создаёт задания со стабильным interval-bucket idempotency key. Scheduler выключен
по умолчанию (`AUTOMATION_SCHEDULER_ENABLED=false`). В Admin UI раздел Automation
показывает worker/scheduler, последние runs и позволяет поставить ручной cycle в
очередь. Distributed lock `automation-cycle:{account_id}` использует owner token и
TTL; параллельный запуск завершается `SKIPPED`.

```powershell
docker compose up --build worker scheduler
curl -Method Post http://localhost:8000/api/v1/admin/automation/run `
  -Headers @{"X-Admin-API-Key"=$env:ADMIN_API_KEY; "Idempotency-Key"="manual-001"}
```
